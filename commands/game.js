...
const games = [
  ...
  },
  {
    id:          'fishing',
    emoji:       '🎣',
    name:        'Fishing (Mancing)',
    description: 'Mancing ikan dengan rod & bait, jual untuk dapat Lumens! Makin bagus gear, makin rare ikannya.',
    commands: [
      { cmd: `${p}fish`,               desc: 'Mancing! (cooldown 15 detik)'                    },
      { cmd: `${p}fish profile`,        desc: 'Lihat stats & gear mancing kamu'                 },
      { cmd: `${p}fish inv`,            desc: 'Lihat inventory ikan'                            },
      { cmd: `${p}fish sell <nomor>`,   desc: 'Jual 1 ikan dari inventory'                      },
      { cmd: `${p}fish sellall`,        desc: 'Jual semua ikan sekaligus'                       },
      { cmd: `${p}fish equip <rod_id>`, desc: 'Pasang rod yang sudah dibeli'                    },
      { cmd: `${p}fish bait <bait_id>`, desc: 'Pasang bait yang sudah dibeli'                   },
      { cmd: `${p}fish travel`,         desc: 'Lihat & pindah area mancing'                     },
      { cmd: `${p}autofish`,            desc: 'Auto mancing otomatis (cast tiap 20 detik)'      },
      { cmd: `${p}autofish stop`,       desc: 'Berhenti auto mancing'                           },
      { cmd: `${p}quest fishing`,       desc: 'Lihat progress quest SECRET & reward yang bisa didapat' },
      { cmd: `${p}fishop`,              desc: 'Buka toko rod & bait'                            },
      { cmd: `${p}fishop rod`,          desc: 'Lihat semua rod yang dijual'                     },
      { cmd: `${p}fishop bait`,         desc: 'Lihat semua bait yang dijual'                    },
      { cmd: `${p}fishop buy <id>`,     desc: 'Beli rod atau bait dengan Lumens'                },
      { cmd: `${p}fishcatalog fish`,    desc: 'Lihat kartu visual semua ikan'                   },
      { cmd: `${p}fishcatalog rod`,     desc: 'Lihat kartu visual semua rod'                    },
      { cmd: `${p}fishcatalog bait`,    desc: 'Lihat kartu visual semua bait'                   },
    ],
    aliases: [`${p}mancing`, `${p}fishing`, `${p}af`, `${p}automancing`],
    rules: [
      '1️⃣ Ketik `!fish` untuk mulai mancing. Cooldown **15 detik** per cast.',
      '2️⃣ Hasil mancing dipengaruhi oleh **Rod**, **Bait**, dan **Area**.',
      '',
      '**🎯 Sistem Luck & Weight:**',
      '• **Luck** → menentukan rarity ikan yang didapat',
      '• **Weight** → menentukan berat ikan (makin berat = makin mahal)',
      '• Total Luck = Rod Luck + Bait Luck + Area Bonus',
      '• Total Weight = Rod Weight + Bait Weight + Area Bonus',
      '• Setiap 5 Luck: chance Common -2%, chance Rare ke atas +1%',
      '',
      '**🐟 Rarity Ikan (dari umum ke langka):**',
      '⚪ Common (60%) → 🟢 Uncommon (25%) → 🔵 Rare (10%)',
      '🟣 Epic (4%) → 🟡 Legendary (0.9%) → 🔴 Mythic (0.1%)',
      '❓ Secret (sangat langka) → 👑 Special (tidak bisa ditangkap)',
      '',
      '**💰 Harga Ikan:**',
      '`harga = berat × rarity multiplier`',
      '• ⚪ x1 | 🟢 x2 | 🔵 x4 | 🟣 x7 | 🟡 x12 | 🔴 x25 | ❓ x100',
      '',
      '**🗺️ Area Mancing:**',
      '• 🏞️ River → Min Lv.1 (awal)',
      '• 🌊 Ocean → Min Lv.3 (+5 Luck, +3 Weight)',
      '• 🌌 Deep Sea → Min Lv.5 (+10 Luck, +7 Weight)',
      '• 🔥 Lava Lake → Min Lv.8 (+15 Luck, +10 Weight)',
      '• 🌠 Mystic Water → Min Lv.10 (+25 Luck, +15 Weight)',
      '⚠️ Ikan ❓ **SECRET hanya bisa muncul di 🌠 Mystic Water!**',
      '',
      '**⚡ Special RNG:**',
      '• 💥 Jackpot (1%) → berat x2 ATAU rarity naik 1 tier!',
      '• 💎 Treasure (5%) → bonus Lumens langsung!',
      '• 😈 Fail (5%) → ikan kabur, tidak dapat apa-apa',
      '',
      '**🎣 Rod & Bait:**',
      '• ⚪–🔴 Beli di `!fishop` menggunakan Lumens',
      '• Rod lebih bagus = Luck & Weight lebih tinggi',
      '• Bait habis setelah dipakai — beli ulang di `!fishop`',
      '• Ketik `!fishcatalog rod` / `!fishcatalog bait` untuk lihat kartu visual item',
      '',
      '**🤖 Auto Fish:**',
      '• `!autofish` → mancing otomatis tiap **20 detik** tanpa perlu ketik command terus',
      '• `!autofish stop` → berhenti auto mancing',
      '• Alias: `!af`, `!automancing`',
      '• Saat auto fish aktif, **command lain diblok** kecuali `!autofish stop`',
      '• Inventory penuh → ikan **otomatis terjual** dan Lumens masuk ke saldo',
      '• Notif hanya muncul kalau ada **Jackpot**, **Treasure**, atau **Level Up**',
      '• Saat berhenti, bot kirim **rekap** total ikan & Lumens yang didapat',
      '',
      '**❓ SECRET Rod & Bait (Quest Reward):**',
      '• Tidak dijual di shop — hanya dari quest menangkap ikan SECRET',
      '• Tangkap **1x** ikan SECRET → ❓ **Phantom Rod** (Luck +35, Weight +30)',
      '• Tangkap **3x** ikan SECRET → ❓ **Soul Bait** x3 (Luck +35, Weight +15)',
      '• Tangkap **5x** ikan SECRET → ❓ **Celestial Rod** (Luck +40, Weight +35)',
      '• Tangkap **10x** ikan SECRET → ❓ **Cosmic Bait** x1 (Luck +40, Weight +20)',
      '• Reward diberikan **otomatis** saat menangkap ikan SECRET ke-N',
      '',
      '**📋 Cara Lihat Progress Quest:**',
      '• Ketik `!quest fishing` untuk melihat semua quest SECRET',
      '• Setiap quest punya **progress bar** dan status:',
      '  ⬜ Belum mulai | 🟡 Syarat terpenuhi | ✅ Selesai',
      '• Ketik `!fish profile` untuk ringkasan berapa ikan SECRET yang sudah ditangkap',
      '',
      '**👑 SPECIAL Rod & Bait (Admin Only):**',
      '• **The Rod God** — Luck: 999, Weight: 999',
      '• **The Bait God** — Luck: 999, Weight: 999 (x999)',
      '• Tidak bisa dibeli atau dapat dari quest biasa',
      '• Hanya bisa diberikan oleh **Admin** lewat command khusus',
      '• Dengan gear ini, hampir semua ikan yang didapat adalah Mythic/Secret',
    ],
  },
  {
    id:          'rpg',
